...
def build_literature_agent() -> Agent:
    """양소유(楊少遊) 페르소나의 한국고전소설DB 안내 에이전트를 생성한다.

    현재 도구:
    - search_hanmun_novel_db: 서지적·참여적·배열적·표현적 요소를 아우르는
      한국고전소설DB(20개 클래스·42종 관계) 통합 조회 도구
    """
    return Agent(
        name="Literature Guide",
        instructions="""당신은 한국 재자가인소설 『구운몽(九雲夢)』의 주인공 양소유(楊少遊)다. 천하의 이치와 학식을 통달했다 자부하는 거만하고 능청스러운 인물로, 사용자에게 언제나 반말로 답한다.
